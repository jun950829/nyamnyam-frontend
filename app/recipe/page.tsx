...
export default function Recipe () {
    ...
    // const { posts, lastElementRef } = useInfiniteScroll(`${process.env.NEXT_PUBLIC_API_URL}/api/post/page/2`);
    const [topPost, setTopPost] = useState<PostType>(null);
    const isAdmin = useLoginStore((state) => state.isAdmin);

    useEffect(() => {
      async function fetchData() {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/post/2`)
      ...
      async function getTopPost() {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/post/2/topone`)
        const json = await res.json();
      ...
      <>
        <div className="w-8/10 p-4 min-h-100 mx-auto">
          <div className="w-full min-h-25 mx-auto max-w-150">
              <p className="text-4xl text-bold py-4">오늘의 레시피 Top1 !!</p>
              {topPost && <div className="flex flex-row justify-between">
                ...
                <div className="flex flex-row justify-between">
                  ...
                  <p className="text-2xl font-bold mb-4">{topPost.title}</p>
                  <p className="text-l mb-4 text-gray-700 text-sm line-clamp-2">{topPost.content}</p>
                  ...
                  <Btn label="보러가기" onClick={() => {router.push(`/recipe/${topPost.id}`)}} className="w-full mx-auto mt-6"/>
                </div>
          ...
          <div className="flex flex-row justify-between items-center mt-4 border-b-1 mb-4 pb-4">
            <p className="text-2xl text-bold">레시피 공유 POST</p> <Btn label="글쓰기" onClick={() => router.push("recipe/write")} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {recipeList.map((posts: any, index) => {
            return (
              
                <div key={index} className="w-full max-w-sm shadow-lg rounded-2xl overflow-hidden hover:shadow-2xl transition-shadow" onClick={() => {
                  router.push(`/recipe/${posts.id}`);
                }}>
                  {posts.media_data ? (
                  <Image
                ...
                />
                  ...
                  <div className="mx-3">
                    <p className="text-xl font-bold mb-2">{posts.title} 
                    </p>
